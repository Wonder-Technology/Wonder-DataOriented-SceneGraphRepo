...
var Result$WonderDataorientedScenegraphrepo = require("./Result.bs.js");
...
function buildFailResult(param) {
  return Result$WonderDataorientedScenegraphrepo.failWith("data not exist in option data");
}

function get(optionData) {
  if (optionData !== undefined) {
    return Result$WonderDataorientedScenegraphrepo.succeed(Caml_option.valFromOption(optionData));
  } else {
    return Result$WonderDataorientedScenegraphrepo.failWith("data not exist in option data");
  }
}
...
function forEachResult(optionData, func) {
  if (optionData !== undefined) {
    ...
    return Result$WonderDataorientedScenegraphrepo.succeed(undefined);
  }
}

function sequenceResultM(optionData) {
  if (optionData !== undefined) {
    return Result$WonderDataorientedScenegraphrepo.mapSuccess(optionData, (function (value) {
