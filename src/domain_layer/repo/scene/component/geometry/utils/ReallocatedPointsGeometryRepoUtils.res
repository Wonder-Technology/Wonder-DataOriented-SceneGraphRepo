...
let getInfo = (infoIndex, infos) =>
  ...
  )->Contract.ensureCheck(((startIndex, endIndex)) => {
    open Contract
    open Operators

    test(Log.buildAssertMessage(~expect=j`has info data`, ~actual=j`not`), () =>
...
let setInfo = (infoIndex, startIndex, endIndex, infos) => Contract.requireCheck(() => {
    open Contract
    open Operators
    test(Log.buildAssertMessage(~expect=j`startIndex >= 0`, ~actual=j`is $startIndex`), () =>
      startIndex >= 0
    )
    test(Log.buildAssertMessage(~expect=j`endIndex >= startIndex`, ~actual=j`is $endIndex`), () =>
      endIndex >= startIndex
