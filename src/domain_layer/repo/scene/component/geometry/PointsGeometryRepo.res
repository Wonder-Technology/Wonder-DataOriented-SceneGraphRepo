...
let getVertexInfo = index => {
  ...
  ReallocatedPointsGeometryRepoUtils.getInfo(
    ...
    (startIndex, endIndex)->Contract.ensureCheck(((startIndex, endIndex)) => {
      ...
      test(
        Log.buildAssertMessage(~expect=j`equal to the info get from normals`, ~actual=j`not equal`),
        () => {
      ...
      test(
        Log.buildAssertMessage(~expect=j`startIndex:$startIndex is 3 times`, ~actual=j`not`),
        () => {
...
let getVertexCount = () => Contract.requireCheck(() => {
    open Contract
    open Operators
    test(Log.buildAssertMessage(~expect=j`verticesOffset be 3 times`, ~actual=j`not`), () => {
