let _updateTransform = cameraController => {
  Tuple5.collectOption(
    ...
    ->OptionSt.bind(GetComponentGameObjectDoService.getTransform),
    OperateArcballCameraContrllerDoService.getDistance(cameraController)
    ->OptionSt.map(DistanceVO.value),
    OperateArcballCameraContrllerDoService.getPhi(cameraController)
    ->OptionSt.map(PhiVO.value),
    OperateArcballCameraContrllerDoService.getTheta(cameraController)
...
let update = () =>
  ...
  ->ListSt.traverseResultM(cameraController =>
      _updateTransform(cameraController)
    )
  ->Result.mapSuccess(_ => {
