...
let computeTangents = (vertices, texCoords, normals, indices) => {
  let vertices = vertices->VerticesVO.value
  let texCoords = texCoords->TexCoordsVO.value
  let normals = normals->NormalsVO.value
  let indices = indices->IndicesVO.value
  ...
  while i.contents < vertexCount {
    ...
    let ndott = Vector3.dot(n, t1)
    let temp = Vector3.scale(Vector3.Float, n, ndott)
    let (tempX, tempY, tempZ) = Vector3.sub(Vector3.Float, t1, temp)->Vector3.normalize
...
let hasTangents = geometry =>
  GeometryRepo.hasTangents(geometry->GeometryEntity.value)
