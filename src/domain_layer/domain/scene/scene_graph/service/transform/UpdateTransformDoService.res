  ...
  DirtyTransformDoService.isDirty(transform)
    ? {
        ...
        | Some(parent) =>
          ...
          let parentLocalToWorldMatrix = ModelMatrixTransformDoService.getLocalToWorldMatrix(parent)
          let childLocalToWorldMatrix = ModelMatrixTransformDoService.getLocalToWorldMatrix(
            transform,
          )
          ...
          ->ignore
        | None =>
          let localToWorldMatrix = ModelMatrixTransformDoService.getLocalToWorldMatrix(transform)
...
let updateAndSetPosition = (transform, position) =>
  switch HierachyTransformDoService.getParent(transform) {
  | None => ModelMatrixTransformDoService.setLocalPosition(transform, position)
  | Some(parent) =>
    mutableUpdate(parent)
...
let updateAndGetRotation = transform => {
  ...
  ModelMatrixTransformDoService.getLocalToWorldMatrix(transform)->LocalToWorldMatrixVO.getRotation
}

let updateAndSetRotation = (transform, rotation) =>
  switch HierachyTransformDoService.getParent(transform) {
  | None => ModelMatrixTransformDoService.setLocalRotation(transform, rotation)
  | Some(parent) =>
    ModelMatrixTransformDoService.setLocalRotation(
      transform,
      updateAndGetRotation(parent)->RotationVO.invert->RotationVO.multiply(rotation),
    )
  }
...
let updateAndGetScale = transform => {
  ...
  ModelMatrixTransformDoService.getLocalToWorldMatrix(transform)->LocalToWorldMatrixVO.getScale
}
