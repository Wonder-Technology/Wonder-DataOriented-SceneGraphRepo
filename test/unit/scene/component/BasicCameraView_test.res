...
open BasicCameraViewTool

let _ = describe("BasicCameraView", () => {
  open Expect
  ...
  describe("active", () =>
    describe("ensure only has one active basicCameraView", () =>
      test("active this one, unactive other ones", () => {
        ...
        (isActive(cameraView1), isActive(cameraView2), isActive(cameraView3))->expect ==
          (false, false, true)
      })
    )
  )
  ...
  describe("unactive", () =>
    test("unactive this one(not affect other ones)", () => {
      ...
      (isActive(cameraView1), isActive(cameraView2))->expect == (true, false)
    })
  )
  ...
  describe("getActiveBasicCameraView", () => {
    test("test has none", () => {
      ...
      getActiveBasicCameraView()->ResultTool.getExnSuccessValue->expect == None
    })
    test("test has one", () => {
      let cameraView1 = create()
      let cameraView2 = create()
      ...
      getActiveBasicCameraView()->ResultTool.getExnSuccessValue->expect == Some(cameraView1)
    })
    test("if has >= 2, contract error", () => {
      ...
      getActiveBasicCameraView()->ExpectTool.toFail("expect only has one active cameraView at most")
    })
  })

  describe("getViewWorldToCameraMatrix", () =>
    test("get vMatrix", () => {
      let (_, transform, (cameraView, cameraProjection)) = CameraTool.createCameraGameObject()

      let pos = (1., 2., 3.)->PositionTool.create
      TransformTool.setLocalPosition(transform, pos)->ResultTool.getExnSuccessValue
      let rotation = (1., 2., 3., 2.5)->RotationTool.create
      TransformTool.setLocalRotation(transform, rotation)->ResultTool.getExnSuccessValue
