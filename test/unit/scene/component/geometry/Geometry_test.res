...
let _ = describe("Geometry", () => {
  ...
  describe("test set points", () => {
    let _testSetVertexDataWithTypeArray = (getFunc, setFunc, createVertexDataVOFunc) =>
      test("directly set it", () => {
        ...
        let newData = Float32Array.make([3., 5., 5.])->createVertexDataVOFunc
    ...
    describe("set vertices with type array", () =>
      _testSetVertexDataWithTypeArray(getVertices, setVertices, VerticesVO.create)
    )
    ...
    describe("set texCoords with type array", () => {
      test("directly set it", () => {
        ...
        setTexCoords(geometry, newData)->ResultTool.getExnSuccessValueIgnore

        getTexCoords(geometry)->ResultTool.getExnSuccessValue->expect == newData
      })
    ...
    describe("set normals with type array", () =>
      _testSetVertexDataWithTypeArray(getNormals, setNormals, NormalsVO.create)
    )

    describe("set tangents with type array", () =>
      _testSetVertexDataWithTypeArray(getTangents, setTangents, TangentsVO.create)
    )
    ...
    describe("set indices with type array", () =>
      test("directly set it", () => {
        ...
        getIndices(geometry)->ResultTool.getExnSuccessValue->expect == newData
      })
    )
  })
  ...
  describe("test geometry has indices", () => {
    ...
    let _testIndices32 = (hasIndicesFunc, result) => {
      ...
      setIndices(
        geometry,
        Uint32Array.make([1, 2, 3])->IndicesVO.create,
      )->ResultTool.getExnSuccessValueIgnore

      hasIndicesFunc(geometry)->ResultTool.getExnSuccessValue->expect == result
    }
    ...
    describe("hasGeometryIndices", () =>
      ...
      // );
      test("if has indices32, return true", () => _testIndices32(hasIndices, true))
    )
  ...
  describe("computeTangents", () =>
    test("sphere's computed tangents shouldn't has NaN", () => {
      let geometry = createSphereGeometry(2.0, 2)->ResultTool.getExnSuccessValue
